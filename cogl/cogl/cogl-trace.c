...
cogl_trace_context_new (void)
{
  CoglTraceContext *context;

  context = g_new0 (CoglTraceContext, 1);
  context->writer = sp_capture_writer_new (COGL_TRACE_OUTPUT_FILE,
                                           4096 * 4);
  return context;
}

static void
ensure_trace_context (void)
{
  static GMutex mutex;

  g_mutex_lock (&mutex);
  if (!cogl_trace_context)
    cogl_trace_context = cogl_trace_context_new ();
                                    ...
                                    gboolean      is_enabled)
{
  ...
  if (is_enabled)
    g_source_set_callback (source, enable_tracing_idle_callback, NULL, NULL);
  else
