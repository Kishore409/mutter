...
#include "transactions-unstable-v1-server-protocol.h"
                            ...
                            struct wl_resource *surface_resource)
{
  ...
  if (g_hash_table_contains (transaction->cached_states, surface))
    {
      wl_resource_post_error (resource, ZWP_TRANSACTION_V1_ERROR_ALREADY_USED,
...
static const struct zwp_transaction_v1_interface meta_wp_transaction_interface = {
...
static const struct zwp_transaction_manager_v1_interface
                          ...
                          uint32_t          id)
{
  ...
  resource = wl_resource_create (client,
                                 &zwp_transaction_manager_v1_interface,
                                 META_ZWP_TRANSACTION_MANAGER_V1_VERSION,
                                 id);
  if (!resource)
    {
      wl_client_post_no_memory (client);
...
meta_wayland_init_transactions (MetaWaylandCompositor *compositor)
{
  ...
  if (!wl_global_create (compositor->wayland_display,
                         &zwp_transaction_manager_v1_interface, 1,
                         NULL,
