  ...
  for (l = page_flip_data->closures; l; l = l->next)
    {
      ...
      if (page_flip_data->is_symbolic)
        {
          closure->iface->ready (page_flip_data->crtc,
                                 closure->user_data);
        }
      else
        {
          closure->iface->flipped (page_flip_data->crtc,
...
void
meta_kms_page_flip_data_set_is_symbolic (MetaKmsPageFlipData *page_flip_data)
{
  ...
  for (l = page_flip_data->closures; l; l = l->next)
    {
      ...
      closure->iface->mode_set_fallback (page_flip_data->crtc,
                                         closure->user_data);
    }
}
  ...
  for (l = page_flip_data->closures; l; l = l->next)
    {
      ...
      closure->iface->discarded (page_flip_data->crtc,
                                 closure->user_data,
