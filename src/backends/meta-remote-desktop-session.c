                      ...
                      MetaLogBuffer  *buf)
{
  ...
  strncpy (buf->buf, mime_types_string,
           G_N_ELEMENTS (buf->buf) - 1);
  if (len >= G_N_ELEMENTS (buf->buf) - 1)
    buf->buf[G_N_ELEMENTS (buf->buf) - 2] = '*';
  buf->buf[G_N_ELEMENTS (buf->buf) - 1] = '\0';
  ...
  if (!session->current_source)
    {
      g_dbus_method_invocation_return_error (invocation, G_DBUS_ERROR,
                                             G_DBUS_ERROR_FAILED,
                                             "Invalid serial serial %u",
                                             serial);
      return TRUE;
    }

  if (serial > session->transfer_serial)
    {
      g_dbus_method_invocation_return_error (invocation, G_DBUS_ERROR,
                                             G_DBUS_ERROR_FAILED,
                                             "Bogus transfer serial %u",
                                             serial);
      return TRUE;
    }
